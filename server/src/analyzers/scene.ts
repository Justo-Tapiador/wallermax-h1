...
export async function analyzeScene(req: PipelineRequest): Promise<AnalysisBundle> {
  ...
  // UNLESS the user explicitly requested a dark mood in their prompt.
  {
    ...
    // v3: Simplest possible dark-mood detection.
    // Step 1: If user EXPLICITLY asks for bright, force bright (period).
    const brightPhrases = [
      "bright", "daylight", "luz brillante", "iluminación brillante",
      "iluminacion brillante", "ambiente brillante", "día brillante",
      "dia brillante", "mood: bright", "mood bright", "luz natural",
      "día soleado", "dia soleado", "sunlight", "luz del sol"
    ];
    const userWantsBright = brightPhrases.some(p => userPrompt.includes(p));

    // Step 2: If user EXPLICITLY asks for dark, check that those words
    // are NOT in a negation context.
    const darkPhrases = ["noir", "melancholic", "cyberpunk", "dystopian",
                          "noche oscura", "dark room", "moody", "low key"];
    let userWantsDark = false;
    if (!userWantsBright) {
      // For each dark phrase, check if it appears NOT preceded by negation
      for (const phrase of darkPhrases) {
        const idx = userPrompt.indexOf(phrase);
        if (idx >= 0) {
          // Look at the 20 characters before the phrase
          const context = userPrompt.substring(Math.max(0, idx - 20), idx);
          const isNegated = /\b(no|sin|avoid|without|not|don'?t|never)\s*$/i.test(context);
          if (!isNegated) {
            userWantsDark = true;
            break;
          }
    ...
    // Strip "nightstand" so "night" inside it doesn't trigger dark detection
    // (this is handled by the darkPhrases list which doesn't include "night" alone)
