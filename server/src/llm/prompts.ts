...
If a final/target image is also provided, treat it as a look-dev reference: extract its camera language, lighting mood, palette and texture feel, then apply them as world.aesthetic and as camera/light/material hints. Do NOT copy the final image's geometry — only its aesthetic. The aesthetic block supports grain, vignette, bloom, chromatic_aberration and color_temperature_k — all map to real compositor stages.

Render quality: set world.render.quality to "cinematic" for final/realistic output (motion blur, raytracing, 128+ samples, AgX color management), "preview" when the user wants speed, "standard" otherwise. For exteriors prefer environment.sky (physical Nishita sky); for interiors an environment.hdri or well-placed AREA lights with size 0.5-2m give soft, believable shadows.
...
Multi-shot camera animation: prefer sequences of camera behaviors over a single static shot. Use behavior.type values "dolly_in", "dolly_out", "crane_up", "crane_down", "pan", "tilt", "orbit", "follow", "spline_path" (smooth Bezier flight through behavior.path waypoints — pair with camera.target or camera.look_at so the subject stays framed), and "zoom_in"/"zoom_out" (animated focal length via lens_start/lens_end). NOT generic "dolly"/"crane". Combine with events on lights (change energy or color over time) for cinematic motion. Always specify behavior.duration; for orbit, also specify behavior.angle and behavior.target. For dolly/crane, specify behavior.distance.
